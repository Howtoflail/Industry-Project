...
{
    // Start is called before the first frame updatepublic float lifetime = 5f; //lifetime of a point on the trail
    ...
    void Update()
    {
        ...
        if (
            points.Count < 2
            || Vector3.Distance(transform.position, points[1]) > minimumVertexDistance
        )
        {
